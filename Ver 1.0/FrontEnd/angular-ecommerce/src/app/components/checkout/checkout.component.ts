...
export class CheckoutComponent implements OnInit {
  ...
  copyShippingAddressToBillingAddress(event) {
    ...
    if (event.target.checked) {
      ...
      //State array is empty cuz we don't select contry manualy when we copy address
      ...
      //Removing the elements when we uncheck the copy option
  ...
  handleMonthsAndYears() {
    ...
    const crediCardFromGroup = this.checkoutFormGroup.get('creditCard');

    const currentYear: number = new Date().getFullYear();
    const selectedYear: number = Number(crediCardFromGroup.value.expirationYear);
